...
import { ISimulationParameters } from "../models/Simulation";
...
export const validateSimulationParams = (
  ...
  const {
    name,
    parameters,
  }: { name?: string; parameters?: ISimulationParameters } = req.body;
  ...
  if (
    typeof parameters.initialPopulation !== "number" ||
    !Number.isInteger(parameters.initialPopulation) ||
    parameters.initialPopulation < 1 ||
    parameters.initialPopulation > 10000
  ) {
    validationErrors.push(
      "Initial population must be an integer between 1 and 10000"
    );
  }

  // Growth Rate validation
  if (
    typeof parameters.growthRate !== "number" ||
  ...
  if (
    typeof parameters.antibioticConcentration !== "number" ||
    parameters.antibioticConcentration < 0 ||
    parameters.antibioticConcentration > 1
  ) {
    validationErrors.push(
      "Antibiotic concentration must be a number between 0 and 1"
    );
  }

  // Mutation Rate validation
  if (
    typeof parameters.mutationRate !== "number" ||
