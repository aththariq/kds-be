...
class SimulationEngine {
    ...
    static calculateNextGeneration(bacteria, params) {
        ...
        const survivors = agedBacteria.filter((bacterium) => {
            const survived = this.applyAntibioticEffect(bacterium, params.antibioticConcentration);
            if (!survived)
                antibioticDeaths++;
            return survived;
        });
        ...
        if (currentPopulation < carryingCapacity) {
            for (const bacterium of livingBacteria) {
                if (this.shouldReproduce(bacterium, params, currentPopulation, carryingCapacity)) {
    ...
    static reproduceBacterium(parent, params) {
        ...
        const newFitness = Math.max(0, Math.min(1, (parent.fitness || 0.5) + fitnessVariation));
        const sizeVariation = (Math.random() - 0.5) * 0.5; // ±0.25 size variation
        const newSize = Math.max(1, Math.min(10, (parent.size || 3) + sizeVariation));
        return {
            id: (0, uuid_1.v4)(),
            x: newPosition.x,
            y: newPosition.y,
    ...
    static processMutations(bacterium, mutationRate) {
        ...
        if (Math.random() < 0.1) {
            ...
            if (mutatedBacterium.isResistant) {
                mutatedBacterium.fitness = Math.max(0, (mutatedBacterium.fitness || 0.5) - 0.1);
            }
            else {
                mutatedBacterium.fitness = Math.min(1, (mutatedBacterium.fitness || 0.5) + 0.1);
            }
            hasMutated = true;
        }
        ...
        if (Math.random() < 0.7) {
            ...
            mutatedBacterium.fitness = Math.max(0, Math.min(1, (mutatedBacterium.fitness || 0.5) + fitnessChange));
            hasMutated = true;
        }
        ...
        if (Math.random() < 0.5) {
            ...
            mutatedBacterium.size = Math.max(1, Math.min(10, (mutatedBacterium.size || 3) + sizeChange));
            hasMutated = true;
        }
    ...
    static calculateStatistics(bacteria) {
        ...
        const averageFitness = totalPopulation > 0
    ...
    static findOffspringPosition(parent, params) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            const distanceFromCenter = Math.sqrt(Math.pow(newX - center.x, 2) + Math.pow(newY - center.y, 2));
...
SimulationEngine.DEFAULT_COLORS = [
